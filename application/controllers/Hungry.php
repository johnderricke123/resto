...
class Hungry extends CI_Controller {
	...
	public function setshipping(){
		...
		$sessiondata = array('shippingmethod' =>$shipname,'shippingrate' =>$shippingcharge);
	    $this->session->set_userdata($sessiondata);
		}
...
document.getElementById("paytrack").click();
  function payWithPaystack(){
    var handler = PaystackPop.setup({
      ...
      amount: "'.round($this->input->post('grandtotal')).'",
      currency: "NGN",
...
$fields3 = array(
	...
	curl_setopt($ch3, CURLOPT_HTTPHEADER, array('Authorization: Key=AAAA4j0CZSQ:APA91bGhEmG9eS2IUjPam6jpDtfBEyvLXGccd_BWGeGolN2pXiVrJ9d06wNut4sXN698cGTgIimXhC6S1CXRnXxRaGmF7n_OvZBK0e3zwqJ1CA6zwRqMaajfxtekvcbaGNfUZmWuRjHZ',
	    'Content-Type: application/json')
	);
	...
	public function paymentgateway($orderid,$paymentid,$page=null){
		  ...
		  if($paymentid==5){
		 	//$this->load->view('sslpayment', $data); 
	...
	public function myprofile(){
			...
			$islogin=$this->session->userdata('CusUserID');
			if(empty($islogin)){redirect('mylogin');}
			else{
			$data['customerinfo'] = $this->hungry_model->read('*', 'customer_info', array('customer_id' => $islogin));
	...
	public function apporedrlist(){
			...
			$islogin=$this->session->userdata('CusUserID');
			if(empty($islogin)){redirect('qr-menu');}
			else{
			$data['iteminfo'] = $this->hungry_model->myorderlist($islogin);
			$data['customerinfo'] = $this->hungry_model->read('*', 'customer_info', array('customer_id' => $islogin));
