                            ...
                            <?php foreach ($purchaselist as $items) { ?>
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
									...
									//echo $newDate = date("d-M-Y", strtotime($originalDate));
                                        echo $originalDate;
									 ?></td>
                                     <td><?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo number_format($items->total_price, 2); ?> <?php if($currency->position==2){echo $currency->curr_icon;}?></td>
                                   <td class="center">
                                    ...
                                    <input name="url" type="hidden" id="url_<?php echo $items->purID; ?>" value="<?php echo base_url("purchase/purchase/updateintfrm") ?>" />
                                        ...
                                        <a href="<?php echo base_url("purchase/purchase/delete/$items->purID") ?>" onclick="return confirm('<?php echo display("are_you_sure") ?>')" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="right" title="Delete "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                         <?php endif; ?>
                                    </td>
