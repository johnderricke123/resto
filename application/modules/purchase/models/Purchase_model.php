...
class Purchase_model extends CI_Model {
	...
	public function create()
	{
		...
		$t_price = $this->input->post('total_price');
      	$p_unit = $this->input->post('purchase_unit');
      	$p_unit_quantity = $this->input->post('purchase_unit_quantity');
		...
		for ($i=0, $n=count($p_id); $i < $n; $i++) {
			...
			$total_price = $t_price[$i];
          	$purchase_unit = $p_unit[$i];
			$purchase_unit_quantity = $p_unit_quantity[$i];
			...
			if(!empty($quantity))
			{
				/*add stock in ingredients*/
              	if($p_unit == "Kilogram" ) {
                  $product_quantity = $product_quantity * $p_unit_quantity;
                }
                if($p_unit == "Liter" ) {
                    $product_quantity = $product_quantity * $p_unit_quantity;
                }
    ...
    public function read($limit = null, $start = null)
	{
		...
		$this->db->join('supplier',' supplier.supid = purchaseitem.suplierID','left');
	...
	public function finditem($product_name)
		{ 
		$this->db->select('ingredients.*, unit_of_measurement.uom_short_code as unit_code');
		$this->db->from('ingredients');
        $this->db->join('unit_of_measurement','unit_of_measurement.id = ingredients.purchase_unit_id','left');
 ...
 public function iteminfo($id){
	 	$this->db->select('purchase_details.*,ingredients.ingredient_name,ingredients.stock_qty,unit_of_measurement.uom_short_code, ingredients.qty_unit');
		$this->db->from('purchase_details');
...
public function countlist()
	{
		...
		/*$this->db->select('purchaseitem.*,supplier.supName,ingredients.ingredient_name,unit_of_measurement.uom_name');
        $this->db->from($this->table);
		$this->db->join('supplier','purchaseitem.suplierID = supplier.supid','left');
		$this->db->join('purchase_details','purchaseitem.purID = purchase_details.purchaseid','left');
		$this->db->join('ingredients','ingredients.id = purchase_details.indredientid','inner');
		$this->db->join('unit_of_measurement','unit_of_measurement.id = ingredients.uom_id','inner');*/
...
public function getinvoice($id){
  ...
  public function get_supplier_ingredients()
	{
