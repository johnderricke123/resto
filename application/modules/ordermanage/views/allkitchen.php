                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-7 customesl">
                                        ...
                                        <input name="mykid" id="mykid" type="hidden" value=""/>
                                      	<input name="itemQty" id="itemQty" type="hidden" value=""/>
                                    </div>
                                </div>
                              <!-- newly added -->
                                <div class="form-group row">
                                    <label for="pinCode" class="col-sm-4 col-form-label"><?php echo "Pin Code";?></label>
                                    <div class="col-sm-7 customesl">
                                      <input type="password" name="pinCode" id="pinCode" class="form-control" cols="35" rows="3"/>
                                                            ...
                                                            <div class="food_item <?php if($isaccept==0){ echo "pending";}?>" id="topsec<?php echo $orderinfo->order_id.$orderinfo->kitchenid;?>">
                                                                <div class="food_item_top">
                                                                    <div class="item_inner">
                                                                        <h4 class="kf_info"><?php echo display('table') ?>: <?php echo $orderinfo->table_no;?></h4>
                                                                ...
                                                                <div class="cooking_time">
                                                                    ...
                                                                    ?>
                                                                    <h4 class="kf_info"><?php echo display('cookedtime') ?>:<?php if($st==1){?><span class="countdown_<?php echo $orderinfo->order_id;echo $c;?>" ><?php //echo $mins;?></span><?php }else{?><span><?php  echo display('time_over');}?></span></h4>
...
<input id='chkbox-<?php echo $l.$item->kitchenid.$orderinfo->order_id;?>' usemap="<?php echo $orderinfo->order_id;?>" title="<?php echo $item->varientid;?>" alt="<?php echo $isaccept;?>" type='checkbox'  <?php if($ischecked==1 && $isaccept==0){ echo "checked disabled";} if($isaccept==1 && $item->food_status==1){ echo "checked";}?> class="<?php echo $item->menuqty; ?>" name="item<?php echo $orderinfo->order_id.$orderinfo->kitchenid;?>" value="<?php echo $item->menu_id;?>"/>
...
<!-- newly added -->
                                                    ...
                                                    <span class="radio-shape">
                                                        <i class="fa fa-check"></i>
                                                                                ...
                                                                                <h4 id="itemQty" class="quantity"><?php echo $item->menuqty;?>x</h4>
                                                                              <input type="hidden" name="item<?php echo $item->menuqty;?>"/>
<!-- newly added -->
                                                    ...
                                                    <span class="radio-shape">
                                                        <i class="fa fa-check"></i>
                                                                        ...
                                                                        <div style="display:<?php if($isaccept==0){ echo "block"; }else{ echo "none";}?>" id="isongoing<?php echo $orderinfo->order_id;?><?php echo $orderinfo->kitchenid;?>">
                                                                            ...
                                                                            <button class="btn btn-danger w-smd lh-30" onclick="ordercancel(<?php echo $orderinfo->order_id;?>,<?php echo $orderinfo->kitchenid;?>,<?php echo $item->menuqty;?>)"><?php echo display('reject') ?></button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
...
<script>
    ...
    function ordercancel(ordid,kitid,itemQty){
        $('#cancelord').modal('show');
        var values = $('input[name="item'+ordid+kitid+'"]:checked:not(:disabled)').map(function() {
            return $(this).val();
        }).get().join(',');
/*
      	var Qty = $('input[name="item'+itemQty+'"]:checked:not(:disabled)').map(function() {
            return $(this).val();
        }).get().join(',');
*/
      
        /*
        var Qty = $('input[name="test'+ordid+kitid+'"]:checked:not(:disabled)').map(function() {
            return $(this).attr('value');
        }).get().join(',');
		*/
        var Qty = $('input[name="item'+ordid+kitid+'"]:checked:not(:disabled)').map(function() {
    ...
    $('body').on('click', '#itemcancel', function() {
      ...
      /*if(pinCode == <?php echo $pass[0]->pass; ?>){*/
      ...
      var itemQty=$("#itemQty").val();
        ...
        var dataString = 'reason='+reason+'&item='+itemid+'&orderid='+ordid+'&varient='+varient+'&kid='+kid+'&pinCode='+pinCode+'&itemQty='+itemQty;
        $.ajax({
            type: "POST",
