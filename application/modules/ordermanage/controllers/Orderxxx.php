        ...
        $updatetordfordiscount = array(
				   ...
				   'customerpaid'           => $this->input->post('grandtotal'),
                    'customer_id'           => $customerid,
          			'note'					=> $this->input->post('anyreason')
				  );
      ...
      //else {
       ...
       // }
		...
		$billinfo = $this->db->select('*')->from('bill')->where('order_id',$orderid)->get()->row();
      	...
      	$discount_amount = $billinfo->discount;
		...
		//echo $paidamount; echo $status;
      	...
      	//	$checkuser = $this->db->select('*')->from('tbl_cashregister')->where('userid',$saveid)->where('status',0)->order_by('id','DESC')->get()->row(); 
      		...
      		if($checkuser) {
              $counter_no = $checkuser->counter_no;
            }
			...
			$total_price =  $cpaidamount;
              ...
              $db_sales = '399';
              $total_price = $total_price - $sales_discount;
            }
      
      		
      		$food_revenue = $cpaidamount / 2;
         	$raw_material = $cpaidamount / 2;
            ...
            if($counter == 1 || $counter == 2) {
      		...
      		if($counter == 3) {
				$db_ledger_id = '276';                  
                    ...
                    $entry['Entry']['notes'] = 'Resto Sale: AR/OR/Invoice #: '.$invoice_no.'<br/><br/>Prepared by: ' . $this->session->userdata('fullname');
