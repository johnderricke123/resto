...
// stock deduction code
  public function get_ingredient_id($pid){
	...
	// var_dump($results[0]->ingredientid);die();
	return $results;
  }
  
    public function insert_stock_deduct($get_ingredient_id, $qty){
	// $this->db->set('stock_deduct', $qty);
	//$this->db->set('stock_deduct', 'stock_deduct+'.$qty);
	$this->db->set('stock_deduct', 'stock_deduct + ' . $qty, false); // Add the false parameter to prevent CodeIgniter from backticks
	$this->db->where('id', $get_ingredient_id->ingredientid);
	$this->db->update('ingredients');
      
// newly added
	$data = array(
		'invoice_id' => $get_ingredient_id->ingredientid,
        'ingredient_id' => $get_ingredient_id->ingredientid,
		'stock_out_qty' => $qty
		
    );

    $this->db->insert('stock_out_trail', $data);
