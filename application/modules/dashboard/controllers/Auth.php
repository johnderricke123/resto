...
class Auth extends MX_Controller {
	...
	//echo $this->session->userdata('captcha');
		...
		#-------------------------------------#
		//echo $this->input->post('captcha');exit;
		$this->form_validation->set_rules('email', display('email'), 'required|min_length[5]|max_length[100]|trim');
...
//print_r($checkPermission);
			...
			if($user->row()->is_admin == 2){
				...
				/*******get user role like counter, waiter, etc. **********/
				//$this->session->userdata('role') - how to access the role from the session variable

				$userId = $this->session->userdata('id');
				$user_access_role = $this->db->select('sec_user_access_tbl.*,sec_role_tbl.*,user.firstname,user.lastname')
				->from('sec_user_access_tbl')
				->where('user.id',$user->row()->id)
				...
				if($user_access_role->role_name){
					$role = $user_access_role->role_name;
				}else{
					$role = 'Admin';
				}
