...
class Home_model extends CI_Model {
	...
	{
        $today=date('Y-m-d');
		$this->db->select('customer_order.*,customer_order.customer_id AS cust_id,customer_info.customer_name,customer_info.customer_phone,rest_table.tablename');
        $this->db->from('customer_order');
...
//newly added
      	$this->db->select('A.*,A.event_name as EventName,A.company_name as CompName,A.meal as EventMeal,A.down_payment as DownPay,A.ar_or as ArOr,A.grand_total as GrandTotal,A.notes as EventNotes,A.customer_phone as EventPhone,customer_info.*,rest_table.tablename');
        $this->db->from('tblreservation A');
		$this->db->join('customer_info','A.cid=customer_info.customer_id','left');
		$this->db->join('rest_table','A.tableid=rest_table.tableid','left');
		$this->db->where('A.status', 2);
		$this->db->order_by('A.reserveday', 'DESC');
		//$this->db->limit(3);
        $query = $this->db->get();
