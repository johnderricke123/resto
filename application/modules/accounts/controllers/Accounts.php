...
class Accounts extends MX_Controller {
    ...
    public function show_tree($id = null){
		...
		//$data['allheadname']=$this->allpheadtable($allphead);
		 $data['title'] = display('act');
		$data['module'] = "accounts";
		$data['page']   = "treeview";   
		echo Modules::run('template/layout', $data); 
 ...
 public function selectphead(){
		 ...
		 /*foreach($coa_phead as $coaphead){
			 	echo '<option value="'.$coaphead->HeadName.'" class="bolden"><strong>'.$coaphead->HeadName.'</strong></option>';
				if(!empty($coaphead->sub)){
						foreach($coaphead->sub as $subcat){
							echo '<option value="'.$subcat->HeadName.'">&nbsp;&nbsp;&mdash;'.$subcat->HeadName.'</option>';
							}
					}
			 }*/
	 }
