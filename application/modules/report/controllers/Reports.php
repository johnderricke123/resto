  ...
  public function productwisereport(){
   ...
   public function ingredientwise(){
	...
	public function salereportbydate()
    {
        ...
        $data['preport']  = $preport = $this->report_model->salereportbydates($start_date,$end_date);
      	
      //	$total_cost = $this->report_model->get_production_cost($start_date,$end_date);
      // var_dump($preport[0]);die();
      $i=0;
      $cost_price=0;
      foreach ($preport as $key => $item) 
    {
       
       $cost = $this->report_model->get_production_cost($item['ProductsID'],$item['variantid']);
            
        if($cost[0]['total_amount'] > 0 ) {
          $cost_price = $cost[0]['total_amount'];
        }
        $data[$key]['total_cost'] = $cost_price;
       $i++;
    }
	...
	public function allsellrpt(){
		...
		$output = array(
						"draw" => $_POST['draw'],
						"cardpayments"=>number_format($card,2,'.',''),
						"Onlinepayment"=>number_format($online,2,'.',''),
						"Cashpayment"=>number_format($cash,2,'.',''),
	...
	public function sellrptCasher(){
		$this->permission->method('report','read')->redirect();
        $data['title']    ='Sales Report by Cashier'; 
		$settinginfo=$this->report_model->settinginfo();
		$data['setting']=$settinginfo;
		$data['currency']=$this->report_model->currencysetting($settinginfo->currency);
        $data['module'] = "report";
		...
		//$first_date = str_replace('/','-',$this->input->post('from_date'));
		$start_date= $this->input->post('from_date'); // date('d-m-Y' , strtotime($first_date));
		// $second_date = str_replace('/','-',$this->input->post('to_date'));
		$end_date= $this->input->post('to_date'); //date('d-m-Y' , strtotime($second_date));
...
//$this->db->from('user');
//$this->db->join('customer_order', 'customer_order.order_id = user.id');
//$this->db->join('bill', 'bill.order_id = customer_order.order_id');
//$this->db->join('payment_method', 'payment_method.payment_method_id = bill.payment_method_id');
//$qreslt = $this->db->get();

//var_dump($start_date);die();                  
//var_dump($this->report_model->order_casher($start_date,$end_date));die();
...
//var_dump($dateRange);die();
    ...
    $first_date = str_replace('/','-',$this->input->post('from_date'));
    $start_date= date('m-d-Y' , strtotime($first_date));
    $second_date = str_replace('/','-',$this->input->post('to_date'));
    $end_date= date('m-d-Y' , strtotime($second_date));
       ...
       foreach ($preports as $value) {
		 /*if($value->isgroup==1){
			 echo $value->order_id."_".$kitchen->kitchenid;
			 $condition="order_menu.order_id=$value->order_id AND order_menu.groupmid=$value->groupmid AND order_menu.groupvarient=$value->groupvarient";
			 $sql="SELECT  DISTINCT(order_menu.groupmid) as menu_id,order_menu.qroupqty,order_menu.isgroup,item_foods.kitchenid FROM order_menu Left JOIN item_foods ON item_foods.ProductsID=order_menu.groupmid WHERE {$condition} AND item_foods.kitchenid=$kitchen->kitchenid AND order_menu.isgroup=1 Group BY order_menu.order_id";
    ...
    $this->load->view('report/kicanwiseReport', $data);
		...
		// $first_date = str_replace('/','-',$this->input->post('from_date'));
		$start_date= $this->input->post('from_date'); // date('m-d-Y' , strtotime($first_date));
	//	$second_date = str_replace('/','-',$this->input->post('to_date'));
		$end_date= $this->input->post('to_date'); // date('m-d-Y' , strtotime($second_date));
  ...
  // stock inventory module code
			public function stock_inventory(){
				...
				$get_orders_ingredients = $this->report_model->get_orders_ingredients();
				
				$data['get_orders_ingredients'] = $get_orders_ingredients;
				$data['get_ingredients'] = $get_ingredients = $this->report_model->get_ingredients();
				//$get_stock_inventory_in = $this->report_model->get_stock_inventory_in();
				$data['date'] = $date; 
				//$data['get_stock_inventory_in'] = $get_stock_inventory_in;
