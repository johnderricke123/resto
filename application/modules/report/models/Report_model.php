...
class Report_model extends CI_Model {
    ...
    //  	$start_date1 = date_format($start_date, "%d-%m-%y");
      
       $start_date1 = date("Y-m-d", strtotime($start_date));  
      	$end_date1 = date("Y-m-d", strtotime($end_date));  
     
		$myarray=array();
		$dateRange = "a.itemid='$pid' AND a.saveddate BETWEEN '$start_date1' AND '$end_date1'";
     
		$this->db->select("a.*,SUM(a.itemquantity) as totalqty, b.ProductsID,b.ProductName as ProductName");
		$this->db->from('production a');
		$this->db->join('item_foods b','b.ProductsID = a.itemid','left');
		$this->db->where($dateRange, NULL, FALSE); 	
		$this->db->order_by('a.saveddate','desc');
		...
		$dateRange2 = "a.menu_id='$pid' AND b.order_date BETWEEN '$start_date1' AND '$end_date1' AND b.order_status!=5";
		$this->db->select("a.*,SUM(a.menuqty) as totalsaleqty,b.order_date");
		$this->db->from('order_menu a');
		$this->db->join('customer_order b','b.order_id = a.order_id','left');
		$this->db->where($dateRange2, NULL, FALSE); 	
		$this->db->order_by('b.order_date','desc');
	...
	public function allingredient(){
		$this->db->select("a.*,SUM(a.quantity) as totalqty, b.id,b.ingredient_name,b.stock_qty,b.stock_deduct,c.uom_short_code");
...
//original code
          $myArray[$i]['Out_Qnty']=$result->stock_deduct.' '.$result->uom_short_code;
//newly added
          	$total = $result->totalqty - $result->stock_deduct;
			$myArray[$i]['Stock']=$total.' '.$result->uom_short_code;
//newly added
	...
	public function salereportbydates($start_date,$end_date)
	{
      ...
      $start_date = date('Y-m-d', strtotime(str_replace('-', '/', $start_date)));
		...
		$sql="SELECT SUM(order_menu.menuqty) as qty,order_menu.order_id,order_menu.groupmid,order_menu.groupvarient,order_menu.isgroup,item_foods.ProductName,item_foods.ProductsID,variant.price,variant.variantName,variant.variantid,customer_order.order_date FROM order_menu Left Join customer_order ON customer_order.order_id=order_menu.order_id LEFT JOIN item_foods ON item_foods.ProductsID=order_menu.menu_id INNER JOIN variant ON variant.variantid=order_menu.varientid Where {$dateRange} AND order_menu.isgroup=0 Group BY order_menu.menu_id,order_menu.varientid UNION SELECT order_menu.qroupqty as qty,order_menu.order_id,order_menu.groupmid,order_menu.groupvarient,order_menu.isgroup,item_foods.ProductName,item_foods.ProductsID,variant.price,variant.variantName,variant.variantid,customer_order.order_date FROM order_menu Left Join customer_order ON customer_order.order_id=order_menu.order_id LEFT JOIN item_foods ON item_foods.ProductsID=order_menu.groupmid INNER JOIN variant ON variant.variantid=order_menu.groupvarient Where {$dateRange} AND order_menu.isgroup=1 Group BY order_menu.groupmid,order_menu.groupvarient";
        ...
        //$this->db->join('production_details','production_details.pvarientid=order_menu.varientid','left');
		$this->db->where($dateRange, NULL, FALSE); 	
	...
	/*22-09*/
	public function salereport($start_date=null,$end_date=null,$pid=null,$invoice_id = null)
	{
		$dateRange = "a.order_date BETWEEN '$start_date' AND '$end_date' AND a.order_status=4";
       // $dateRange = "a.order_date BETWEEN "'. date('Y-m-d', strtotime($start_date)). '" and "'. date('Y-m-d', strtotime($end_date));
      ...
      //	$this->db->where('a.order_date >=', date('Y-m-d',strtotime($start_date)));
		//$this->db->where('a.order_date <=', date('Y-m-d',strtotime($end_date)));
...
/*22-09*/
	...
	public function order_items($ids,$catid=null){
		...
		$sql="SELECT SUM(order_menu.menuqty) as totalqty,order_menu.order_id,order_menu.groupmid,order_menu.groupvarient,order_menu.isgroup,item_foods.ProductName,variant.price,variant.variantName FROM order_menu LEFT JOIN item_foods ON order_menu.menu_id=item_foods.ProductsID LEFT JOIN variant ON order_menu.varientid=variant.variantid WHERE {$condition} AND order_menu.isgroup=0 GROUP BY order_menu.menu_id,order_menu.varientid UNION SELECT order_menu.qroupqty as totalqty,order_menu.order_id,order_menu.groupmid,order_menu.groupvarient,order_menu.isgroup,item_foods.ProductName,variant.price,variant.variantName FROM order_menu LEFT JOIN item_foods ON order_menu.groupmid=item_foods.ProductsID LEFT JOIN variant ON order_menu.groupvarient=variant.variantid WHERE {$condition} AND order_menu.isgroup=1 GROUP BY order_menu.groupmid,order_menu.groupvarient";
		...
		if(!empty($catid)){
		$this->db->where_in('item_foods.CategoryID',$catid);	
		}
	...
	public function order_casher($start_date,$end_date){
      ...
      $this->db->join(' payment_method z','z.payment_method_id=c.payment_method_id','left');
		$this->db->where($dateRange, NULL, FALSE);
...
//var_dump($dateRange);die();
        ...
        if ($query->num_rows() > 0) {
    		$row = $query->row();
    		$createByValue = $row->create_by;
		}
      
      
      //var_dump($createByValue);die();
	...
	public function itemsKiReport($kid,$start_date,$end_date)
	{
		...
		$sql="SELECT customer_order.saleinvoice, order_menu.*, item_foods.kitchenid, variant.price FROM order_menu LEFT JOIN customer_order ON customer_order.order_id=order_menu.order_id LEFT JOIN item_foods ON item_foods.ProductsID=order_menu.menu_id INNER JOIN variant ON variant.menuid=item_foods.ProductsID WHERE {$dateRange} AND order_menu.isgroup=0 UNION SELECT customer_order.saleinvoice, order_menu.*, item_foods.kitchenid, variant.price FROM order_menu LEFT JOIN customer_order ON customer_order.order_id=order_menu.order_id LEFT JOIN item_foods ON item_foods.ProductsID=order_menu.menu_id INNER JOIN variant ON variant.menuid=item_foods.ProductsID WHERE {$dateRange} AND order_menu.isgroup=1 GROUP BY order_menu.groupmid,order_menu.addonsuid";
		$this->db->select("customer_order.saleinvoice,order_menu.*,item_foods.kitchenid,variant.price");
		$this->db->from('order_menu');
		$this->db->join('customer_order','customer_order.order_id=order_menu.order_id','left');
		$this->db->join('item_foods','item_foods.ProductsID=order_menu.menu_id','left');
    ...
    public function all_hotel_guests()
    {
        $hoteldb = $this->load->database('hmsdb', TRUE);

      	$result	= $hoteldb->get('guests');
	...
	function get_hotel_guest($id)
    {
  	...
  	function get_production_cost($foodid=null, $pvareintid=null,$start_date=null, $end_date=null){
      
     
      $dateRange = "production_details.created_date BETWEEN '$start_date' AND '$end_date'";
      $this->db->select("*, SUM(total_price) as total_amount");			
		if($pvareintid){
           $this->db->where('pvarientid',$pvareintid); 
        }
      if($foodid){
           $this->db->where('foodid',$foodid); 
        }	
      $this->db->group_by('foodid,pvarientid');
  ...
  // stock inventory module code
 	function get_stock_inventory_in(){
		...
		$this->db->select('A.*,B.*,B.indredientid as pur_ing_id');
		$this->db->from('ingredients A');
		$this->db->where('B.purchasedate', $date);
		$this->db->join('purchase_details B','B.indredientid = A.id');
		$ingredients_query = $this->db->get();
		$ingredients = $ingredients_query->result();

		//var_dump($ingredients);die();
		return $ingredients;
	}

	function get_stock_inventory_in_stock($id, $date){
		//$date = date('Y-m-d');
		// $date = '2024-02-13';
		$this->db->select('A.*, A.price, A.quantity, A.purchasedate');
		$this->db->where('indredientid',$id);
		$this->db->where('purchasedate', $date);
		$query = $this->db->get('purchase_details A');
	...
	// function get_stock_inventory_out_stock($id, $date){
	// 	$this->db->select('SUM(A.stock_out_qty) as stock_out');
	// 	$this->db->where('ingredient_id',$id);
	// 	$query = $this->db->get('stock_out_trail A');
	// 	$results = $query->row();
	// 	return $results->stock_out;
	// }

	// function get_stock_out($id, $date){
	// 	$this->db->select('SUM(stock_out_trail.stock_out_qty) as stock_out');
	// 	$this->db->where('ingredient_id', $id);
	// 	$this->db->where('created_at', $date);
	// 	$query = $this->db->get('stock_out_trail');
	// 	$results = $query->row();
	...
	// }
  	...
  	function get_stock_out($id, $date){
		...
		//$this->db->select('stock_out_trail.stock_out_qty');
		$this->db->select('SUM(stock_out_trail.stock_out_qty) as stock_out');
		$this->db->where('ingredient_id', $id);
		$this->db->where('created_at', $date);
		$query = $this->db->get('stock_out_trail');
	...
	function get_beginning($ingredient_id,$date){
		//$date_convert = date('Y-m-d', strtotime($date . ' -1 day'));
		//var_dump($date_convert);die();

		//$date = "2024-03-09";
		//$ingredient_id = 803;

		$beginning = array();
		$this->db->select('A.*, A.price, A.quantity');
		$this->db->where('indredientid', $ingredient_id);
		$this->db->where('purchasedate <', $date);
// newly added
		//$this->db->where('purchasedate > 2024-02-01');
// newly added
		$query = $this->db->get('purchase_details A');
		$beginning['purchase_details'] = $query->first_row();
		$purchase_total_quantity = $this->get_purchase_total($ingredient_id, $date);
		$beginning['quantity_total'] = $purchase_total_quantity; 

		$date_deducted = $date = date('Y-m-d', strtotime($date . ' -1 day'));
		$stock_deduct = $this->get_stock_out($ingredient_id,$date_deducted);
		$beginning['stock_deduct'] = $stock_deduct;

		return $beginning;
	}

// newly added
	function get_purchase_total($ingredient_id, $date){
		$this->db->select('SUM(purchase_details.quantity) as quantity');
		$this->db->where('indredientid', $ingredient_id);
		$this->db->where('purchasedate <', $date);
		$query = $this->db->get('purchase_details');
		$result = $query->result();
		//var_dump($result[0]->quantity);die();
		return $result[0]->quantity;
	}
// newly added

	public function get_orders_ingredients(){

		$date = $this->input->post('date') ?: date('Y-m-d');
		//var_dump($date);die();
		//$date = date('Y-m-d', strtotime("2024-01-01" . ' -1 day'));

		$this->db->select('A.itemid,B.ingredientid,B.qty,B.unitname, C.ingredient_name, D.order_id, D.order_date');
		$this->db->where('D.order_date', $date);
		$this->db->join('customer_order D','D.order_id = A.orderid');
		$this->db->join('production_details B', 'B.foodid = A.itemid');
		$this->db->join('ingredients C','C.id = B.ingredientid');
		//$this->db->join('purchase_details E', 'E.indredientid = A.itemid');
		$this->db->from('tbl_kitchen_order A');
		$query = $this->db->get();
		$results = $query->result();
		
		//var_dump($results[0]->order_date);die();
		...
		foreach($results as $result){
			...
			$this->db->select('A.quantity, A.price');
			$this->db->where('indredientid', $result->ingredientid);
			$this->db->where('purchasedate <', $date);
			$query2 = $this->db->get('purchase_details A');
			$result2 = $query2->last_row();
			//var_dump($result->ingredientid."/////".$result2->quantity);
			if(!$informations[$result->ingredientid]['date'][$i] && !$informations[$result->ingredientid]['ingredient_name'][$i] && !$informations[$result->ingredientid]['purchase_quantity'][$i]){
				
				$informations[$result->ingredientid]['date'] = $result->order_date;
				$informations[$result->ingredientid]['ingredient_name'] = $result->ingredient_name;
				$informations[$result->ingredientid]['purchase_quantity'] += $result2->quantity;
				$informations[$result->ingredientid]['purchase_price'] += $result2->price;
			}

			$this->db->select('A.quantity, A.price');
			$this->db->where('purchasedate', $date);
			$this->db->where('indredientid', $result->ingredientid);
			$query3 = $this->db->get('purchase_details A');
			$result3 = $query3->result();
			// echo "//////".$result->ingredientid;
			// var_dump($result3);
			// echo "//////";
			if($result3){
				$informations[$result->ingredientid]['in_stock_purchase_quantity'][0] = $result3[0]->quantity;
				$informations[$result->ingredientid]['in_stock_purchase_price'][0] = $result3[0]->price;
			}else{
				$informations[$result->ingredientid]['in_stock_purchase_quantity'][0] += $result3[0]->quantity;
				$informations[$result->ingredientid]['in_stock_purchase_price'][0] += $result3[0]->price;
			}
			...
			$out_stock_qty = 0;
			//$total_ingredient_sold = $this->get_total_ingredient_sold(12, $date);
			//var_dump($total_ingredient_sold);die();
			foreach($results as $rs ){
				if($rs->ingredientid == $result->ingredientid){
					$out_stock_qty += $rs->qty;
				}
			}

			$informations[$result->ingredientid]['out_stock_purchase_quantity'][0] = number_format($out_stock_qty, 2);
			$informations[$result->ingredientid]['out_stock_purchase_price'][0] = $result2->price;
	...
	//	die();
		...
		//var_dump($informations[713]);die();
		return $informations;
		//echo json_encode($informations);die();
