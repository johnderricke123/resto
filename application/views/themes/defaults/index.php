...
<?php if($whatsapp_count  == 1){
   ...
   <?php 
    ...
    <script src="<?php //echo base_url();?>application/views/themes/<?php //echo $acthemename; ?>/assets_web/plugins/gmap/map-active.js"></script>-->
    <?php if($this->settinginfo->site_align=='RTL'){?>
	   ...
	   var getdate=$("#reservation_date").val();
		...
		var currentDate = new Date();
           ...
           if(checkhour >= inputtime ){
               alert("Please select after 1 hour to Current time!!!");
                    return false;
              }
...
setTimeout(function(){ x.className = x.className.replace("snackbar show", "snackbar"); }, 3000);
	...
	function addnotetoitem(){
		  ...
		  var geturl='<?php echo base_url();?>hungry/additemnote';
		  var dataString = "foodnote="+note+'&rowid='+rowid;
		  $.ajax({
		 	 type: "POST",
			 url: geturl,
			 data: dataString,
